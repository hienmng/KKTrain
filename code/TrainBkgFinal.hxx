//Code generated automatically by TMVA for Inference of Model file [TrainBkgFinal.h5] at [Wed May 17 19:14:09 2023] 
...
std::vector<float> fTensor_dense1bias0bcast = std::vector<float>(12);
...
std::vector<float> fTensor_dense2bias0bcast = std::vector<float>(12);
float * tensor_dense2bias0bcast = fTensor_dense2bias0bcast.data();
std::vector<float> fTensor_densebias0bcast = std::vector<float>(12);
...
Session(std::string filename ="") {
   ...
   f.close();
   {
      float * data = TMVA::Experimental::SOFIE::UTILITY::UnidirectionalBroadcast<float>(tensor_densebias0,{ 12 }, { 1 , 12 });
      std::copy(data, data + 12, tensor_densebias0bcast);
      delete [] data;
   }
   {
      float * data = TMVA::Experimental::SOFIE::UTILITY::UnidirectionalBroadcast<float>(tensor_dense1bias0,{ 12 }, { 1 , 12 });
      std::copy(data, data + 12, tensor_dense1bias0bcast);
      delete [] data;
   }
   {
      float * data = TMVA::Experimental::SOFIE::UTILITY::UnidirectionalBroadcast<float>(tensor_dense2bias0,{ 12 }, { 1 , 12 });
      std::copy(data, data + 12, tensor_dense2bias0bcast);
      delete [] data;
   }
   {
...
//--------- Gemm
   ...
   int op_0_lda = 6;
   int op_0_ldb = 12;
   std::copy(tensor_densebias0bcast, tensor_densebias0bcast + 12, tensor_denseDense);
...
//--------- Gemm
   ...
   int op_2_lda = 12;
   int op_2_ldb = 12;
   std::copy(tensor_dense1bias0bcast, tensor_dense1bias0bcast + 12, tensor_dense1Dense);
...
//--------- Gemm
   ...
   int op_4_lda = 12;
   int op_4_ldb = 12;
   std::copy(tensor_dense2bias0bcast, tensor_dense2bias0bcast + 12, tensor_dense2Dense);
